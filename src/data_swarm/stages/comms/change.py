...
class CommsChangeAgent:
    """Comms Change Agent."""

    name = "Comms Change Agent"

    def generate(self, task_id: str, critic_eval: dict, curator_candidates: dict, home: Path) -> str:
        """Generate comms change request and update repetition signals."""
        store = StagePolicyStore(home, "comms")
        store.ensure_scaffold()
        suggestions = critic_eval.get("suggestions", [])
        keys = [s.get("suggestion_key", "comms_suggestion") for s in suggestions]
        counts = store.update_repetition_index(keys)
        lines = ["# Comms Change Request", "", "## Suggested changes"]
