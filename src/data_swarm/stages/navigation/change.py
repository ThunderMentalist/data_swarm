"""Navigation change request agent."""
...
class NavigationChangeAgent:
    """Navigation Change Agent."""

    name = "Navigation Change Agent"

    def generate(self, task_id: str, critic_eval: dict, curator_candidates: dict, home: Path) -> str:
        """Generate change request markdown and update repetition signals."""
        store = StagePolicyStore(home, "navigation")
        store.ensure_scaffold()
        suggestions = critic_eval.get("suggestions", [])
        keys = [s.get("suggestion_key", "navigation_suggestion") for s in suggestions]
        counts = store.update_repetition_index(keys)
        lines = ["# Navigation Change Request", "", "## Suggested changes"]
