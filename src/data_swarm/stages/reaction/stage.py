...
class ReactionStage(AgenticStage):
    ...
    def run(self, task: Task, task_dir: Path, kb: dict | None = None, attachments: list[dict] | None = None) -> StageResult:
        ...
        harness = StageHarness(StageSpec("reaction", "06_reaction", "initial_feedback.json", "draft_feedback.json", "final_feedback.json", []), self.io, self.store, self.logs, self.anonymizer)
