...
def test_config_normalizes_legacy_llm_model(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo = tmp_path / "data_swarm"
    repo.mkdir()
    (tmp_path / "meridian").mkdir()
    (tmp_path / "meridian_aux").mkdir()
    home = tmp_path / ".data_swarm"
    home.mkdir()
    config_text = f'{{"paths": {{"repo_root": "{repo.as_posix()}"}}, "llm": {{"model": "legacy-model"}}}}'
    (home / "config.yaml").write_text(config_text, encoding="utf-8")
    monkeypatch.setenv("DATA_SWARM_HOME", str(home))

    cfg = load_config(home)
    assert cfg.payload["llm"]["defaults"]["model"] == "legacy-model"
    assert "model" not in cfg.payload["llm"]
