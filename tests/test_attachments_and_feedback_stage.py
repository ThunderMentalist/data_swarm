...
def test_reaction_stage_writes_triage_patch(tmp_path: Path) -> None:
    ...
    task_dir = store.create(task)
    io = FakeIO(answers=["Decision was approved.", "END", "1"])
    stage = ReactionStage(config={}, home=home, io=io, store=store, logs=LogStore(task_dir))
    result = stage.run(task, task_dir)
    assert result.approved is True
    assert (task_dir / "06_reaction" / "triage_update_patch.json").exists()
